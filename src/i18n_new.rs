...
lazy_static! {
    static ref I18N_INSTANCE: RwLock<Option<I18n>> = RwLock::new(None);
}

pub fn init() -> Result<()> {
    let i18n = I18n::new()?;
    let mut instance = I18N_INSTANCE.write().unwrap();
    *instance = Some(i18n);
    Ok(())
}
...
pub fn set_locale(locale: &str) -> Result<()> {
    let mut instance = I18N_INSTANCE.write().unwrap();
    if let Some(ref mut i18n) = *instance {
...
pub fn t(key: &str) -> String {
    let instance = I18N_INSTANCE.read().unwrap();
    if let Some(ref i18n) = *instance {
        i18n.get(key)
    } else {
        key.to_string()
    }
